...
def makeCombi(directory, inputFile, process) :
  chain = TChain("ttbbLepJets/tree")
  chain.Add(inputFile)

  entries = chain.GetEntries()
  for i in xrange(chain.GetEntries()) :
    ptp.printProgress(i, chain.GetEntries(), 'Progress:','Complete',1,25)
    chain.GetEntry(i)
	...
	if chain.jet_CSV[j] > jet_CSV_tight and chain.jet_CSV[k] > jet_CSV_tight:
	  ...
	  jetCombi.append([
	      i,dR,abs(b1.Eta()-b2.Eta()),b1.DeltaPhi(b2),
  ...
  io.save(directory+"/ntuple_"+process+".h5",combi)
