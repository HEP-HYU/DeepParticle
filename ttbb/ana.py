...
def ana(directory, inputFile, process) :
  ...
  for i in xrange(chain.GetEntries()):
    ...
    if len(dataset) is not 0 :
      ...
      pred = model.predict(inputset, batch_size=1000)
      maxval = np.argmax(pred, axis = 0)
     
      reco_addbjet1.SetPtEtaPhiE(
	chain.jet_pT[combidR[maxval[0]][0]],chain.jet_eta[combidR[maxval[0]][0]],
	chain.jet_phi[combidR[maxval[0]][0]],chain.jet_E[combidR[maxval[0]][0]])
      reco_addbjet2.SetPtEtaPhiE(
	chain.jet_pT[combidR[maxval[0]][1]],chain.jet_eta[combidR[maxval[0]][1]],
	chain.jet_phi[combidR[maxval[0]][1]],chain.jet_E[combidR[maxval[0]][1]])
    ...
    isMatch_DNN = False
    isMatch_mindR = False
    isMatch_DNN = (reco_addbjet1.DeltaR(gen_addbjet1) < 0.5 and reco_addbjet2.DeltaR(gen_addbjet2)) or (reco_addbjet1.DeltaR(gen_addbjet2) < 0.5 and reco_addbjet2.DeltaR(gen_addbjet1))
    isMatch_mindR = (reco_addbjet1_mindR.DeltaR(gen_addbjet1) < 0.5 and reco_addbjet2_mindR.DeltaR(gen_addbjet2)) or (reco_addbjet1_mindR.DeltaR(gen_addbjet2) < 0.5 and reco_addbjet2_mindR.DeltaR(gen_addbjet1))
